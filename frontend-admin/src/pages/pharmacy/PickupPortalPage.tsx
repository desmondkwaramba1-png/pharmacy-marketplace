...
export default function PickupPortalPage() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!bookingRef || bookingRef.length < 4) return;

    setIsLoading(true);
    setOrder(null);
    try {
      const data = await adminApi.getOrder(bookingRef);
      setOrder(data);
    } catch (err: any) {
  ...
  const handleCompletePickup = async () => {
    ...
    try {
      await adminApi.collectOrder(order.booking_ref);
        ...
        {order ? (
          <div className="medicine-card" style={{ cursor: 'default', border: '1px solid var(--color-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: 1 }}>{order.booking_ref}</div>
                <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>Reserved on {new Date(order.created_at).toLocaleString()}</div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              <div style={{ padding: 12, background: 'var(--color-bg)', borderRadius: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 600 }}>
                  <FiUser /> {order.user_id ? 'Authenticated Patient' : 'Guest Customer'}
                </div>
              </div>
              <div style={{ padding: 12, background: 'var(--color-bg)', borderRadius: 8 }}>
                <div style={{ fontSize: 11, color: 'var(--color-text-secondary)', textTransform: 'uppercase', marginBottom: 4 }}>Total to Collect</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--color-primary)' }}>${order.total_amount}</div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {order.items?.map((item: any) => (
                  <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--color-border)' }}>
                    <div>
                      <div style={{ fontWeight: 500 }}>{item.medicine?.generic_name}</div>
                      <div style={{ fontSize: 11, color: 'var(--color-text-secondary)' }}>{item.medicine?.brand_name} ({item.medicine?.dosage})</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 600 }}>qty: {item.quantity}</div>
                      <div style={{ fontSize: 11 }}>${item.price_at_booking} ea</div>
                    </div>
                  </div>
                ))}
              </div>
