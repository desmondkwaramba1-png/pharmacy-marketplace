...
import { FiX, FiClock, FiTrash2, FiShoppingBag, FiInfo } from 'react-icons/fi';
...
export default function CartDrawer() {
  ...
  return (
    <>
      <div className="modal-overlay" onClick={() => setCartOpen(false)} style={{ zIndex: 1000 }} />
      <div 
        style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <button className="btn-icon" onClick={() => setCartOpen(false)}><FiX /></button>
        </div>
        ...
        {items.length > 0 && (
          <div style={{ padding: '20px', borderTop: '1px solid var(--color-border)', background: 'var(--color-bg)' }}>
            ...
            <button className="btn btn-primary btn-full" onClick={() => alert('Checkout not implemented yet!')}>
              Confirm & Book
            </button>
          </div>
        )}
