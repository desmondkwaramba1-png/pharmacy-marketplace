...
// POST /api/compliance/run-scraper  (admin-only, protected by AdminGuard middleware)
...
export async function verifyPharmacy(req: Request, res: Response) {
  ...
  if (!premise) {
    await prisma.pharmacy.update({
      where: { id: pharmacyId },
      data: { mcazVerified: false, mcazSuspended: true, marketplaceStatus: 'suspended',
  ...
  if (!addressMatch) {
    ...
    return res.json({ verified: false, reason: 'Address mismatch with MCAZ register' });
  }

  await prisma.pharmacy.update({
    where: { id: pharmacyId },
    data: { mcazVerified: true, mcazSuspended: false, marketplaceStatus: 'active',
             mcazVerifiedAt: new Date(), mcazSuspendReason: null },
  });
  return res.json({ verified: true, premise });
}

// GET /api/compliance/cart-check?pharmacyId=&medicineIds[]=
// Returns which medicines in the cart require a prescription for this pharmacy
export async function cartComplianceCheck(req: Request, res: Response) {
  ...
  // Check pharmacy is still active
