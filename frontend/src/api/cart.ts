...
export const cartApi = {
  ...
  checkout: async (
    ...
    // Use the same cart ID that was used to add items — do NOT call
    // getOrCreateCartId() fresh here, as it can resolve a different cart
    // after login/session transitions, causing the "Cart is empty" error.
    if (!cachedCartId) {
      cachedCartId = await getOrCreateCartId();
    }
    const cartId = cachedCartId;
    ...
    if (!cartItems || cartItems.length === 0) {
      // Try to give a more useful error: check if cart has items at all
      const { data: anyItems } = await supabase
        .from('cart_items')
        .select('id, pharmacy_id, status')
        .eq('cart_id', cartId)
        .limit(5);
      const detail = anyItems?.length
        ? `Cart has items but not for this pharmacy (IDs: ${anyItems.map(i => i.pharmacy_id).join(', ')}, status: ${anyItems.map(i => i.status).join(', ')})`
        : 'Cart is completely empty';
      throw new Error(`Cart is empty for this pharmacy. ${detail}`);
    }

    // Get pharmacy-specific prices from inventory (more accurate than standard_price)
    ...
    const priceMap = new Map((inventoryPrices || []).map(p => [p.medicine_id, Number(p.price)]));

    const subtotal = cartItems.reduce((sum, item) => {
      const price = priceMap.get(item.medicine_id) ?? Number(item.medicine?.standard_price ?? 0);
    ...
    // 3. Create Order Items (use pharmacy-specific price, fall back to standard_price)
    await supabase.from('order_items').insert(
      cartItems.map(ci => ({
        order_id: order.id,
        medicine_id: ci.medicine_id,
        quantity: ci.quantity,
        price_at_booking: priceMap.get(ci.medicine_id) ?? Number(ci.medicine?.standard_price ?? 0),
      }))
    );
